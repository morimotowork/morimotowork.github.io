/*
Script developed by Júnior MORIMOTO
    ...
    */
    ...
    if (window.matchMedia("(max-width:768px)").matches) {
        ...
        window.addEventListener('orientationchange', function(){
            if (window.matchMedia("(orientation: portrait)").matches) {
                alert("Use a aplicação verticalmente para uma melhor experiência.");
            }
        });
    ...
    $('.sec2').find('button').click(function(){
        ...
        if ($('.sec').find('input').first().is( ":checked" ) == false){
            ...
            alertHeader("Colagem sem cola e nem tesoura?");

        } else if ($('.sec2').find('input:checked').length < 3 && $('.sec').find('input').first().is( ":checked" ) == true){ 

            var nf = 3 - $('.sec2').find('input:checked').length;
            alertHeader("Selecione mais " + nf + " opção(es) de materiais para uma melhor combinação.");
